...
/**
 * Deletes DNS records from Hostinger for the configured zone.
 * 
 * @param {Array<Object>} records Array of records to delete (e.g. [{ name: "testdev-2", type: "CNAME" }])
 * @returns {Promise<boolean>}
 */
export async function deleteDnsRecords(records) {
  const token = process.env.HOSTINGER_API_TOKEN;
  const domain = process.env.HOSTINGER_DOMAIN || "clientesneurolinks.com";
